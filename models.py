...
class SIRNetwork(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        r_N = (x[:, 2]).reshape(-1, 1)
        p_N = (x[:, 3]).reshape(-1, 1)
        return s_N, i_N, r_N, p_N

    def parametric_solution(self, t, initial_conditions, beta=None, gamma=None):
        # Parametric solutions
        t_0 = 0
        s_0, i_0, r_0, p_0 = initial_conditions[0][:], initial_conditions[1][:], initial_conditions[2][:], \
        ...
        t_bundle = torch.cat([t, i_0, r_0, p_0, beta, gamma], dim=1)
    ...
    def solve(self, i_0, r_0, p_0, beta, gamma, t_0=0, t_final=20, size=100):
        ...
        p_0_t = torch.Tensor([p_0]).reshape(-1, 1)

        initial_conditions_set = [s_0_t, i_0_t, r_0_t, p_0_t]
        ...
        for i, t in enumerate(t_dl, 0):
            ...
            # Network solutions
            s, i, r, p = self.parametric_solution(t, initial_conditions_set, beta=beta_t, gamma=gamma_t)
            s_hat.append(s.item())
            i_hat.append(i.item())
            r_hat.append(r.item())
            p_hat.append(p.item())
